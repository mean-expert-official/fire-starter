...
export class TodoComponent implements OnDestroy {
  ...
  refresh() {
    this.subscriptions.push(this.todoApi.find().subscribe(
      (todos: Todo[]) => {
        this.todos = todos;
      }));
  }
  ...
  handleAction(event) {
    switch (event.type) {
      case 'create':
        this.subscriptions.push(this.todoApi.create(event.payload).subscribe(
          () => {
            this.refresh();
            this.modalRef.close();
        ...
        break;
      case 'update':
        this.subscriptions.push(this.todoApi.upsert(event.payload).subscribe(
          () => {
            this.refresh();
            this.modalRef.close();
        ...
        break;
      case 'delete':
        this.subscriptions.push(this.todoApi.deleteById(event.payload.id).subscribe(
          () => {
            this.refresh();
